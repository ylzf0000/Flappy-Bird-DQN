...
class AgentDQN:
    def __init__(self, args: RLArguments):
        ...
        # 当前值网络, 目标网络
        self.policy_net = DuelingDeepNetwork(
            layer_num=2,
            seq_len=self.args.state_frames,
            embed_dim=24,
            input_dim=12,
            num_heads=3,
        ).to(self.device)
    ...
    def test(self):
        ...
        total_reward = 0
        terminated = False
        states_after = MultiFrameState(self.args.state_frames)
    ...
    def train(self):
        ...
        for episode in range(100000):
            ...
            local_time_step = 0
            terminated = False
            states = MultiFrameState(self.args.state_frames)
            states.push(state)
            while not terminated:
                local_time_step += 1
                global_time_step += 1
                if local_time_step % self.args.decision_interval != 0:
                    ...
                    action = self.get_action_softmax(env, states.tolist(), local_time_step)
                ...
                states = copy.deepcopy(states_after)
